...
and the coach.
  ...
  python -m calculator.play              # interactive
  python -m calculator.play --demo       # scripted walk (no input), prints everything
...
from calculator.scope import IN_SCOPE_STEPS, Coverage, Insured, Tariff, route
...
def _print_report(log: ActivityLog) -> None:
    from calculator.eventproc import collapse, detections, features
    from calculator.tlm import encode
    ...
    print("── TLM TOKENS ──", len(encode(log, persona="unknown")), "ids "
          f"(vocab={__import__('calculator.tlm', fromlist=['VOCAB']).VOCAB.__len__()})")
